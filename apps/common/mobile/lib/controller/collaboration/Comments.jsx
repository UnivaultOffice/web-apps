...
class CommentsController extends Component {
    ...
    changeCommentData (id, data) {
        ...
        const date = (data.asc_getUnivaultOfficeTime()) ? new Date(stringOOToLocalDate(data.asc_getUnivaultOfficeTime())) :
        ...
        for (let i = 0; i < repliesCount; ++i) {
            ...
            dateReply = (data.asc_getReply(i).asc_getUnivaultOfficeTime()) ? new Date(stringOOToLocalDate(data.asc_getReply(i).asc_getUnivaultOfficeTime())) :
    ...
    readSDKReplies (data) {
        ...
        if (repliesCount) {
            for (i = 0; i < repliesCount; ++i) {
                date = (data.asc_getReply(i).asc_getUnivaultOfficeTime()) ? new Date(stringOOToLocalDate(data.asc_getReply(i).asc_getUnivaultOfficeTime())) :
                    ((data.asc_getReply(i).asc_getTime() === '') ? new Date() : new Date(stringUtcToLocalDate(data.asc_getReply(i).asc_getTime())));
                const userId = data.asc_getReply(i).asc_getUserId();
                const user = this.usersStore.searchUserById(userId);
                const userName = data.asc_getReply(i).asc_getUserName();
...
class AddCommentController extends Component {
    ...
    onAddNewComment (commentText, documentFlag) {
        ...
        if (commentText.length > 0) {
            ...
            comment.asc_putUnivaultOfficeTime(ooDateToString(new Date()));
...
class EditCommentController extends Component {
    ...
    onChangeComment (comment) {
        ...
        if (ascComment && comment) {
            ...
            ascComment.asc_putUnivaultOfficeTime(ooDateToString(new Date(comment.time)));
            ...
            if (reply && reply.length > 0) {
                reply.forEach((reply) => {
                    ...
                    if (addReply) {
                        ...
                        addReply.asc_putUnivaultOfficeTime(ooDateToString(new Date(reply.time)));
    ...
    onAddReply (comment, replyVal) {
        ...
        if (ascComment) {
            ...
            if (addReply) {
                ...
                addReply.asc_putUnivaultOfficeTime(ooDateToString(new Date()));
